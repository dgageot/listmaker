...
public class ListMakerTest {
	...
	@Test
	public void canExecutionActionForEachElement() {
		final List<Integer> all = Lists.newArrayList();

		with(1, 2, 3, 4).forEach(new Action<Integer>() {
			@Override
		...
		assertThat(all).containsExactly(1, 2, 3, 4);
	}

	@Test
	public void canExecutionActionForEachElementWithIndex() {
		final List<String> indices = Lists.newArrayList();

		with("A", "B", "C").forEach(new ActionWithIndex<String>() {
			@Override
